...
% File Name: main_3_5.m
% Created On: 3/31/2026

% Output: finding the real eigenvalues that correspond to the gains needed for task
% 3.5
...
%% K3 sweep ranges
K3_lat_vec = linspace(1e-7, 5e-4, 4000);
K3_lon_vec = linspace(1e-7, 5e-4, 4000);
...
for i = 1:length(K3_lat_vec)
    ...
    % Require: real poles, stable, slowest pole <= -0.8
    if max(abs(imag(lam))) < 1e-8 && all(real(lam) < 0) && max(real(lam)) <= -0.8
...
if ~isempty(valid_lat)
    ...
    A_lat_choice = [ 0                g               0;
                     0                0               1;
                    -K3_lat_choice/Ix -K2_lat/Ix   -K1_lat/Ix ];
...
if ~isempty(valid_lon)
    ...
    A_lon_choice = [ 0                -g              0;
                     0                 0              1;
                     K3_lon_choice/Iy -K2_lon/Iy   -K1_lon/Iy ];
